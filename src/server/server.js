...
const setResponse = (html, preloadedState, manifest) => {
  ...
  const vendoBuild = manifest ? manifest['vendors.js'] : 'assets/vendor.js';
  return `
    <!DOCTYPE html>
      <html lang="en">
      <head>
      ...
      <body>
        ...
        <script src=${vendoBuild} type=""text/javascript> </script>
      </body>
  </html>`;
};
...
const renderApp = async (req, res) => {
  ...
  const html = renderToString(
    <Provider store={store}>
      <StaticRouter location={req.url} context={{}}>{renderRoutes(serverRoutes(isLogged))}</StaticRouter>
    </Provider>,
  );
...
app.delete('/user-movies/:userMovieId', async (req, res, next) => {
  try {
    ...
    const listDelete = userMovies.filter(
      (movie) => movie.movieId === userMovieId
    );
    const { data, status } = await axios({
